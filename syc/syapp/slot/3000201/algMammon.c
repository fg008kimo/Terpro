...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		for(i = 0; i < ret.rotaryNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.rotaryData[i]), ROTARY_ICON_NUM));
		cJSON_AddItemToObject(opRet, "rotaryData", arrJson);
		cJSON_AddItemToObject(opRet, "rotaryMultis", cJSON_CreateIntArray((const int*)ret.rotaryMultis, ret.rotaryNum));
		cJSON_AddItemToObject(opRet, "hitTarget", cJSON_CreateIntArray((const int*)ret.hitTarget, ret.rotaryNum));
		cJSON_AddNumberToObject(opRet, "buffCounter", ret.buffCounter);
	}
