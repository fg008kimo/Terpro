...
uint32_t GetRand_2(uint32_t minValue,uint32_t maxValue)
{
	return RngGetRand(minValue, maxValue);
																			 ...
																			 //in board_handle_mulIcon(), there is a caller: RandSelect(mulIconProb, MUL_ICON_TYPE, PROB_SUM), mulIconProb = NULL
int RandSelect(const uint32_t* probArray, uint16_t probNum, uint32_t probSum)//in freegame.c: probArray = gFreeGameMulIconProb1 = {7498,1502,1000}, probNum = 3, probSum = PROB_SUM = 10000
{
	int i;
	uint32_t randNum = GetRand(probSum) + 1;//+1把[0,probSum)转为[1,probSum]  //probSum = 10000, randNum = 0 ~ 10000取一個數+1

	for (i = 0; i < probNum; i++)//i = 0~2
    {
        if (randNum <= probArray[i])
        {
            printf("i:%d, randNum:%d, probArray[i]:%d, %s\n", i, randNum, probArray[i], __FUNCTION__);
			return i;
        }
        randNum -= probArray[i];
    }
