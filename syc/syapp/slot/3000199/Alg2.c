	...
	s16 freegameNum, s16 *freegameMulIconNums)
{
	...
	for(i = 0; i < normalMulIconNum; i++)
		mulIconTotalMulti += normalMulIconMultis[i];//normalMulIconMultis = 單次base game的炸彈倍率
	mulIconTotalMulti = (mulIconTotalMulti == 0 ? 1 : mulIconTotalMulti);
	for(i = 0; i < normalCascadeNum; i++)
		totalMulti += normalMultis[i] * mulIconTotalMulti;//每次玩的base game總分 = base game分數(一般水果的倍率總和) * 炸彈倍率(但base game不會有炸彈)
	printf("basegame total: mulIconTotalMulti:%d, totalMulti:%d, %s\n", mulIconTotalMulti, totalMulti, __FUNCTION__);
	...
	if(prizeType == E_PRIZETYPE_FREEGAME)
	{
		for(i = 0; i < freegameNum; i++)
		{
			...
			for(j = 0; j < freegameMulIconNums[i]; j++)
				mulIconTotalMulti += freegameMulIconMultis[i][j];//free game炸彈倍率相加
			mulIconTotalMulti = (mulIconTotalMulti == 0 ? 1 : mulIconTotalMulti);
			for(j = 0; j < freegameCascadeNums[i]; j++)
				totalMulti += freegameMultis[i][j] * mulIconTotalMulti;//base game總分加完,再加free game的總分(totalMulti = basegame倍率 * basegame炸彈倍率 + freegameMultis倍率 * freegame炸彈倍率)
			printf("freegame total: i:%d, mulIconTotalMulti:%d, totalMulti:%d, %s\n", i, mulIconTotalMulti, totalMulti, __FUNCTION__);
		}
	}
