...
#define TOTAL_ROUNDS    (1000000000) //7879212 = 197*198*202 (for tranversal use)
...
//enum dy_symbol_payoutDist_types {DY_SYMBOL_PAYOUTDIST_TYPE_NORMAL, DY_SYMBOL_PAYOUTDIST_TYPE_BUY_NORMAL};
...
void dy_handle(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
	...
	for(type = 0; type < DY_SYMBOL_PAYOUTDIST_TYPE; type++)
	{
		...
		//if((type >= DY_SYMBOL_PAYOUTDIST_TYPE_BUY_NORMAL) ||
		...
		if(type % 2 == 0)
		{
			for(i = 0; i < ret->normalLineNum; i++)
			{
				...
				printf("icon:%d, dySymbolPayoutDistWin[type][icon]:%lf, ret->normalLineInfos[i][2]:%d, line:%d in dy_handle()\n", icon, dySymbolPayoutDistWin[type][icon], ret->normalLineInfos[i][2], __LINE__);
				#endif
			}
		}
	}
...
void dy_print()
{
	...
	//LOGI("游戏整体RTP: %.7f%%\n", dyTotalWin / dyTotalBet * 100);  //for tranversal use
	...
	LOGI("\n[图标开奖分布]");
	//char *titles2[DY_SYMBOL_PAYOUTDIST_TYPE] = {"一般游戏:", "一般游戏(购买游戏时):"};
	...
	for(type = 0; type < DY_SYMBOL_PAYOUTDIST_TYPE; type++)
	{
		...
		for(icon = E_ICON_PIPC1; icon <= E_ICON_WILD; icon++)
		{
			...
			//LOGI("| %5s | %7.7f%% |", symbolStr[icon],  //for tranversal use
...
STR_AlgKillFishResult* protocol_mammonKillFish_2(int curBetLineNum, int wantTableId)//wantTableId = 1
{
