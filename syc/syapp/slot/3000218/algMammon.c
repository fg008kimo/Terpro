...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	char errBuf[STR_LEN_128] = {0};
	char *json_out = NULL;
	cJSON *root = NULL, *opRet = NULL, *itemJson = NULL, *arrJson = NULL, *arrJson2 = NULL, *arrJson3 = NULL;
	...
	itemJson = cJSON_GetObjectItem(dataJson, "totalBetScore");
	if (itemJson != NULL)
		val.totalBetScore = (s64)itemJson->valuedouble;
	else
	{
		strcpy(errBuf, "totalBetScore is NULL");
		goto errorDone;
	}
	...
	for(i = 0; i < ret.normalCascadeNum; i++)
		cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.normalIcons[i]), BOARD_ICON_NUM));
	cJSON_AddItemToObject(opRet, "normalIcons", arrJson);
	cJSON_AddItemToObject(opRet, "normalMultis", cJSON_CreateIntArray((const int*)ret.normalMultis, ret.normalCascadeNum));
	arrJson = cJSON_CreateArray();
	for(i = 0; i < ret.normalCascadeNum; i++)
	{
		arrJson2 = cJSON_CreateArray();
		for(j = 0; j < ret.normalRewardIconTypes[i]; j++)
		{
			cJSON_AddItemToArray(arrJson2, cJSON_CreateIntArray((const int*)(ret.normalRewardInfos[i][j]), 3));
		}
		cJSON_AddItemToArray(arrJson, arrJson2);
	}
	cJSON_AddItemToObject(opRet, "normalRewardInfos", arrJson);
	cJSON_AddItemToObject(opRet, "normalMulIconMultis", cJSON_CreateIntArray((const int*)ret.normalMulIconMultis, ret.normalMulIconNum));
	if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		{
			arrJson2 = cJSON_CreateArray();
			for(j = 0; j < ret.freegameCascadeNums[i]; j++)
			{
				cJSON_AddItemToArray(arrJson2, cJSON_CreateIntArray((const int*)(ret.freegameIcons[i][j]), BOARD_ICON_NUM));
			}
			cJSON_AddItemToArray(arrJson, arrJson2);
		}
		...
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.freegameMultis[i]), ret.freegameCascadeNums[i]));
		cJSON_AddItemToObject(opRet, "freegameMultis", arrJson);
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
		{
			arrJson2 = cJSON_CreateArray();
			for(j = 0; j < ret.freegameCascadeNums[i]; j++)
			{
				arrJson3 = cJSON_CreateArray();
				for(k = 0; k < ret.freegameRewardIconTypes[i][j]; k++)
					cJSON_AddItemToArray(arrJson3, cJSON_CreateIntArray((const int*)(ret.freegameRewardInfos[i][j][k]), 3));
				cJSON_AddItemToArray(arrJson2, arrJson3);
			}
			cJSON_AddItemToArray(arrJson, arrJson2);
		}
		cJSON_AddItemToObject(opRet, "freegameRewardInfos", arrJson);
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.freegameMulIconMultis[i]), ret.freegameMulIconNums[i]));
		cJSON_AddItemToObject(opRet, "freegameMulIconMultis", arrJson);
	}
