...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	cJSON_AddNumberToObject(opRet, "multiplier", ret.multiplier);
...
int main(int argc, char *argv[])
{
    ...
    else
    {
		...
		//LOGI("go handle_start_setting();");
		handle_start_setting(alg.port);
	
	//LOGI("go pthread_mutex_init();");
        pthread_mutex_init(&mutex, NULL);    

        //LOGI("go alg_hb_thread_create()");
