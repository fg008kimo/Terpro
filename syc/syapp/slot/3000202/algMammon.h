...
typedef struct{
	s32 times;                 // lineBet,buyFreegameScore被放大了times倍
	s64 lineBet;			   // 每条线的押分（协议字段名bet）
	s16 lineNum;               // 有押分的线数（协议字段名betLineNum）
	s32 tableId;
	s64 buyFreegameScore;      // 购买免费游戏价格，非购买时传0,购买时传50*lineBet*lineNum
}STR_AlgKillFish;
...
typedef struct{
	...
	// 基础牌面，所有中奖类型都有
	int *normalIconsBefore;
	int *normalIcons;      	  				  // 图标列表（一维数组，长度15）,將3x5盤面0~14記錄起來
	u32 normalMulti;                     	  // 倍率
	int (*normalLineInfos)[4];	  			  // 得分线（二维数组[[线id,得分图标id,線得分,倍率符]]）
	
	// prizeType=1
	//int (*freegameIconsBefore)[BOARD_ICON_NUM];  		// 每局变化前的图标列表（二维数组）
	int (*freegameIcons)[BOARD_ICON_NUM];	  	 		// 每局的图标列表（二维数组）
	int (*freegameMultiplier)[BOARD_COLUMN];  	 		// 倍率符
	int *freegameMultis;                      	 		// 每局的倍率（一维数组）
	int (*freegameLineInfos)[BET_LINE_TOTAL][4]; 		// 每局的得分线（三维数组[[[线id,得分图标id,線得分,倍率符]]]）
	
	//int (*freegameIconsBeforeSticky)[BOARD_ICON_NUM];  	// 每局变化前的图标列表（二维数组）
	int (*freegameIconsSticky)[BOARD_ICON_NUM];	  		// 每局的图标列表（二维数组）
	int *freegameMultisSticky;                      	// 每局的倍率（一维数组）
	int (*freegameLineInfosSticky)[BET_LINE_TOTAL][4];	// 每局的得分线（三维数组[[[线id,得分图标id,線得分,倍率符]]]）

	s16 normalLineNum;         		// 算法专用
	s16 freegameNum;		  		// 算法专用
	s16 *freegameLineNum;	   		// 算法专用
	s16 freegameNumSticky;		    // 算法专用
	s16 *freegameLineNumSticky;	    // 算法专用
}STR_AlgKillFishResult;

typedef struct{
	s8 prizeType;
	u32 totalMulti;
	int multiplier[BOARD_COLUMN];							  

	int normalIconsBefore[BOARD_ICON_NUM];
	int normalIcons[BOARD_ICON_NUM];
	u32 normalMulti;
	int normalLineInfos[BET_LINE_TOTAL][4];

	//int freegameIconsBefore[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameIcons[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameMultiplier[FREEGAME_ROUND_MAX][BOARD_COLUMN];
	int freegameMultis[FREEGAME_ROUND_MAX];
	int freegameLineInfos[FREEGAME_ROUND_MAX][BET_LINE_TOTAL][4];
	
	//int freegameIconsBeforeSticky[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameIconsSticky[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameMultisSticky[FREEGAME_ROUND_MAX];
	int freegameLineInfosSticky[FREEGAME_ROUND_MAX][BET_LINE_TOTAL][4];

	s16 normalLineNum;
	s16 freegameNum;
	s16 freegameLineNum[FREEGAME_ROUND_MAX];
	s16 freegameNumSticky;
	s16 freegameLineNumSticky[FREEGAME_ROUND_MAX];
