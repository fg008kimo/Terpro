...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	cJSON_AddItemToObject(opRet, "multiplier", cJSON_CreateIntArray((const int*)ret.multiplier, BOARD_COLUMN));
	...
	if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		//	cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.freegameIconsBefore[i]), BOARD_ICON_NUM));
		//cJSON_AddItemToObject(opRet, "freegameIconsBefore", arrJson);
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
