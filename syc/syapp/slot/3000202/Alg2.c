...
int get_totalWinMulti(s8 prizeType, u32 normalMulti, int *freegameMultis, s16 freegameNum, int *freegameMultisSticky, s16 freegameNumSticky)
{
	int totalMulti;
	int i;
	...
	if(prizeType == E_PRIZETYPE_STICKY_WILD)
	{
		for(i = 0; i < freegameNumSticky; i++){  
			totalMulti += freegameMultisSticky[i];
			#ifdef DEBUG_PAYOUT_DIST
			printf("totalMulti:%d, freegameMultisSticky[i]:%d, %s\n", totalMulti, freegameMultisSticky[i], __FUNCTION__);
			#endif
		}
	}
	//if(prizeType == E_PRIZETYPE_BOBUSGAME)
	//{
	//	for(i = 0; i < BonusNum; i++){
	//		totalMulti += BonusMultis[i];
	//		#ifdef DEBUG_PAYOUT_DIST
	//		printf("totalMulti:%d, BonusMultis[i]:%d, %s\n", totalMulti, BonusMultis[i], __FUNCTION__);
	//		#endif
	//	}
	//}
...
void cpy_result(STR_AlgKillFishResult* target, STR_AlgKillFishResultTemp* source)
{
	...
	if(source->prizeType == E_PRIZETYPE_STICKY_WILD)
	{
		target->freegameIconsSticky = source->freegameIconsSticky;
		target->freegameMultisSticky = source->freegameMultisSticky;
		target->freegameLineInfosSticky = source->freegameLineInfosSticky;
		target->freegameNumSticky = source->freegameNumSticky;
		target->freegameLineNumSticky = source->freegameLineNumSticky;
	}
...
void alg_kill_fish(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
	...
	if(is_dajiang_mode()) //大奖模式
	{
		if(isBuyFreegame || GetRandom() >= 0.5)
			board_gen(gBuyFreeGameRollerIcon, gBuyFreeGameRollerIconNum, gResult.normalIconsBefore);
		else
			board_gen(gRollerIcon, gRollerIconNum, gResult.normalIconsBefore);
	}
	else
	{
		if(isBuyFreegame)
	...
	/*
	//logic  //use to test tranverse
	if(is_dajiang_mode()) //大奖模式
	{
		if(isBuyFreegame || GetRandom() >= 0.5)
			board_traverse_gen(gBuyFreeGameRollerIcon, gBuyFreeGameRollerIconNum, gResult.normalIconsBefore, gResult.multiplier);
		else
			board_traverse_gen(gRollerIcon, gRollerIconNum, gResult.normalIconsBefore, gResult.multiplier);
	}
	else
	{
		if(isBuyFreegame)
	...
	*/
	...
	genMultiplier(gBaseGameWeight, BASEGAME_WEIGHT_SUM, gResult.multiplier);
	memcpy(gResult.normalIcons, gResult.normalIconsBefore, sizeof(gResult.normalIconsBefore));
	IconsToVSsymbol(gResult.normalIconsBefore, gResult.normalIcons);
	gResult.normalMulti = board_calMulti(val->lineNum, gResult.normalIcons, gResult.normalLineInfos, &gResult.normalLineNum, gResult.multiplier);
	...
	if(gResult.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		freeGame_gen(isBuyFreegame, gResult.freegameNum, gResult.freegameIcons, gResult.freegameMultiplier);
		for(round = 0; round < gResult.freegameNum; round++){
			gResult.freegameMultis[round] = board_calMulti(val->lineNum, gResult.freegameIcons[round], gResult.freegameLineInfos[round], &gResult.freegameLineNum[round], gResult.freegameMultiplier[round]);
			#ifdef DEBUG_PAYOUT_DIST
			printf("gResult.freegameMultis[round]:%d, line:%d in alg_kill_fish()\n", gResult.freegameMultis[round], __LINE__);
			#endif
		}
	}
	if(gResult.prizeType == E_PRIZETYPE_STICKY_WILD)
	{
		int round;
		gResult.freegameNumSticky = FREE_GAME_NUM;//can test the sticky syndrome is correct or not
		//gResult.freegameNumSticky = 1;//can test the score summation is correct or not
		freeGame_genSticky(isBuyFreegame, gResult.freegameNumSticky, gResult.freegameIconsSticky);
		for(round = 0; round < gResult.freegameNumSticky; round++){
			gResult.freegameMultisSticky[round] = board_calMulti(val->lineNum, gResult.freegameIconsSticky[round], gResult.freegameLineInfosSticky[round], &gResult.freegameLineNumSticky[round], 0);
			#ifdef DEBUG_PAYOUT_DIST
			printf("round:%d, gResult.freegameMultisSticky[round]:%d, line:%d in alg_kill_fish()\n", round, gResult.freegameMultisSticky[round], __LINE__);
			#endif
		}
	}
	
	//if(gResult.prizeType == E_PRIZETYPE_BOBUSGAME)
	//{
	//	int round;
	//	gResult.freegameNumBonus = freeGame_calNum(gResult.normalIconsBefore);
	//	freeGame_genBonus(isBuyFreegame, gResult.freegameNumBonus, gResult.freegameIconsBefore, gResult.freegameIcons, gResult.freegameMultiplier);
	//	for(round = 0; round < gResult.freegameNumBonus; round++){
	//		gResult.freegameMultisBonus[round] = board_calMulti(val->lineNum, gResult.freegameIcons[round], gResult.freegameLineInfos[round], &gResult.freegameLineNum[round], gResult.freegameMultiplier[round]);
	//		#ifdef DEBUG_PAYOUT_DIST
	//		printf("gResult.freegameMultisBonus[round]:%d, line:%d in alg_kill_fish()\n", gResult.freegameMultisBonus[round], __LINE__);
	//		#endif
	//	}
	//}

	//maxSend
	gResult.totalMulti = get_totalWinMulti(gResult.prizeType, gResult.normalMulti, gResult.freegameMultis, gResult.freegameNum, gResult.freegameMultisSticky, gResult.freegameNumSticky);
