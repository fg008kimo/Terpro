...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	if(ret.normalLineNum > 0)
	{
		...
		for(i = 0; i < ret.normalLineNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.normalLineInfos[i]), 4));
	...
	if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.scArray[i]), BOARD_ICON_NUM));
		cJSON_AddItemToObject(opRet, "scArray", arrJson);
		...
		for(i = 0; i < ret.freegameNum; i++)
		{
			...
			{
				cJSON_AddItemToArray(arrJson2, cJSON_CreateIntArray((const int*)(ret.freegameLineInfos[i][j]), 4));
			}
    ...
    cJSON_AddNumberToObject(opRet, "linebet", val.lineBet);
    cJSON_AddNumberToObject(opRet, "lineNum", val.lineNum);
