...
typedef struct{
	s32 times;                 // lineBet,buyBounsScore被放大了times倍
	s64 lineBet;			   // 每条线的押分
	s16 lineNum;               // 有押分的线数
	s32 tableId;
	s8 blackDiff;              // 算法难度值
	...
	s8 uid;
	int playerDiff;            // 用户难度
	s64 buyFreegameScore;      // 购买ROSS/MAXX免费游戏价格，非购买时传0，购买时传100*lineBet*lineNum
}STR_AlgKillFish;

typedef struct{
	s8 prizeType;			   				  // 中奖类型：0-普通游戏、1-免费游戏
	s32 totalMulti;            			      // 总倍率（等于normalMulti+freegameMultis[]）

	// 基础牌面，所有中奖类型都有
	int *normalIconsAgo;      	  			  // 图标列表[前]（一维数组，长度15）
	int *normalIcons;      	  				  // 图标列表[后]（一维数组，长度15）
	int normalMulti;                          // 倍率
	int (*normalLineInfos)[3];	  		      // 得分线（二维数组[[线id,得分图标id,倍率]]）
	int *normalWildMultis;                    // 普通游戏奖励翻倍每一列的百搭倍率
	
	// prizeType=1
	int (*freegameIconsAgo)[BOARD_ICON_NUM];	     // 每局的图标列表[前]（二维数组）
	int (*freegameIcons)[BOARD_ICON_NUM];	         // 每局的图标列表[后]（二维数组）
	int *freegameMultis;                             // 每局的倍率（一维数组）
	int (*freegameLineInfos)[BET_LINE_TOTAL][3];     // 每局的得分线(三维数组[[[线id,得分图标id,倍率]]])
	int (*freeWildMultis)[BOARD_COLUMN];             // 免费游戏奖励翻倍每一列的百搭倍率

	int normalLineNum;         // 算法专用
	int freegameNum;		   // 算法专用
	int *freegameLineNum;	   // 算法专用
...
typedef struct{
	...
	int normalWildMultis[BOARD_COLUMN];

	int freegameIconsAgo[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameIcons[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameMultis[FREEGAME_ROUND_MAX];
	int freegameLineInfos[FREEGAME_ROUND_MAX][BET_LINE_TOTAL][3];
	int freeWildMultis[FREEGAME_ROUND_MAX][BOARD_COLUMN]; 

	int normalLineNum;
	int freegameNum;
	int freegameLineNum[FREEGAME_ROUND_MAX];
