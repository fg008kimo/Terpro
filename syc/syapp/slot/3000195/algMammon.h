...
typedef struct{
	...
	// prizeType=1
	int (*freegamePreIcons)[BOARD_ICON_NUM];  // 每局变化前的图标列表（二维数组）, preIcon就是那四個神PIC1~PIC4, BOARD_ICON_NUM = 15(盤面一維矩陣)
	int (*freegameIcons)[BOARD_ICON_NUM];	  // 每局的图标列表（二维数组）
	int (*freegameChangeWildMode)[HIGH_ICON_NUM]; // 每局的[宙斯,波塞冬,阿瑞斯,雅典娜]是否变白搭1是0否（二维数组）
...
typedef struct{
	...
	int freegamePreIcons[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameIcons[FREEGAME_ROUND_MAX][BOARD_ICON_NUM];
	int freegameChangeWildMode[FREEGAME_ROUND_MAX][HIGH_ICON_NUM];
