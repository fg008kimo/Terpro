...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		for(i = 0; i < ret.freegameNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.freegamePreIcons[i]), BOARD_ICON_NUM));
		cJSON_AddItemToObject(opRet, "freegamePreIcons", arrJson);
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
		...
		arrJson = cJSON_CreateArray();
		for(i = 0; i < ret.freegameNum; i++)
			cJSON_AddItemToArray(arrJson, cJSON_CreateIntArray((const int*)(ret.freegameChangeWildMode[i]), HIGH_ICON_NUM));
		cJSON_AddItemToObject(opRet, "freegameChangeWildMode", arrJson);
