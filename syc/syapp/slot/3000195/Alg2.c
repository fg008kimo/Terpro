...
void cpy_result(STR_AlgKillFishResult* target, STR_AlgKillFishResultTemp* source)
{
	...
	{
		target->freegamePreIcons = source->freegamePreIcons;
		target->freegameIcons = source->freegameIcons;
		target->freegameChangeWildMode = source->freegameChangeWildMode;
...
void alg_kill_fish(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
	...
	printf("########################################start game##################################\n");
	printf("totalBet:%f, singleLineBet:%f\n", totalWin, singleLineBet);

	//logic
	if(is_dajiang_mode()) //大奖模式
	{
		if(isBuyFreegame || GetRandom() >= 0.5)
			board_gen(gBuyFreeGameRollerIcon, gBuyFreeGameRollerIconNum, gResult.normalIcons);
		else
			board_gen(gRollerIcon, gRollerIconNum, gResult.normalIcons);
			//gRollerIcon = base game reel背景輪帶表的所有icon 另還有gBuyFreeGameRollerIcon(freeGame是另個function), gRollerIconNum = 每條reel的數量(1x5矩陣)(例如base game={47,49,53,63,60}) 
			//,normalIcons是1x15矩陣,表示這個盤0~14的值
	}
	else
	{
		if(isBuyFreegame)
			board_gen(gBuyFreeGameRollerIcon, gBuyFreeGameRollerIconNum, gResult.normalIcons);//board_gen() = 產生輪盤,此case 產生 buyfree game board
		else
			board_gen(gRollerIcon, gRollerIconNum, gResult.normalIcons);//產生base game board, gResult.normalIcons為1x15的矩陣, 紀錄3x5的盤面
	}
	...
	gResult.normalMulti = board_calMulti(val->lineNum, gResult.normalIcons, gResult.normalLineInfos, &gResult.normalLineNum);//算base game盤面分數, 為何有&,請看C語言筆記
	if(gResult.prizeType == E_PRIZETYPE_FREEGAME)//進入FREE GAME
	{
		printf("start free game\n");
		int round;
		gResult.freegameNum = freeGame_calNum(gResult.normalIcons);//計算可以玩幾次freeGame
		freeGame_gen(isBuyFreegame, gResult.freegameNum, gResult.freegamePreIcons, gResult.freegameIcons, gResult.freegameChangeWildMode);//board_gen()+freeGame_changeWildMode()
		for(round = 0; round < gResult.freegameNum; round++)
		{
			printf("calculate the %d round free game multi\n", round);
			...
			printf("gResult.freegameMultis[round]:%d\n", gResult.freegameMultis[round]);
		}
		printf("end free game\n");
	}

	//maxSend
	gResult.totalMulti = get_totalWinMulti(gResult.prizeType, gResult.normalMulti, gResult.freegameMultis, gResult.freegameNum);//前面先計算完normalMulti(base game分數),這邊加上free game分數
