...
void handle_mammon_kill_fish(struct connect_client *client, cJSON *dataJson)
{
	...
	itemJson = cJSON_GetObjectItem(dataJson, "betScore");
	if (itemJson != NULL)
		val.betScore = (s64)itemJson->valuedouble;
	else
	{
		strcpy(errBuf, "betScore is NULL");
		goto errorDone;
	} 
	
	itemJson = cJSON_GetObjectItem(dataJson, "set");
	if (itemJson != NULL)
		val.set = (s64)itemJson->valueint;
	else
	{
		strcpy(errBuf, "set is NULL");
		goto errorDone;
	} 
	...
	cJSON_AddItemToObject(opRet, "normalpreIcons", cJSON_CreateIntArray((const int*)ret.normalpreIcons, BOARD_ICON_NUM));
	cJSON_AddItemToObject(opRet, "respinIcons", cJSON_CreateIntArray((const int*)ret.respinIcons, BOARD_ICON_NUM));
	cJSON_AddNumberToObject(opRet, "normalMulti", ret.normalMulti);

	//if(ret.prizeType == E_PRIZETYPE_RESPINGAME)
	//{
	cJSON_AddNumberToObject(opRet, "respinMulti", ret.respinMulti);
	//}
	//if(ret.prizeType == E_PRIZETYPE_FREEGAME)
	//{
	cJSON_AddNumberToObject(opRet, "freegameMultis", ret.freegameMultis);
	//}

    cJSON_AddNumberToObject(opRet, "betScore", val.betScore);
    cJSON_AddNumberToObject(opRet, "set", val.set);
...
#ifdef __DEBUG__
		//int i, run_times = 5;
		//for(i = 0; i < run_times; i++) //if needs run more than once with same data experiment <=但rtp成績會隨著後面次數衰退
		test_main();
#endif
		_mammon_main_loop();
