...
void cpy_result(STR_AlgKillFishResult* target, STR_AlgKillFishResultTemp* source)
{
	...
	target->betScore = source->betScore;
...
void alg_kill_fish(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
	...
	if (val->betScore <= 0)
	{
		LOGE("function: %s, Line: %d, val->betScore error: %d\n", __FUNCTION__, __LINE__, val->betScore);
	}
	...
	//cal in
	totalBet = (double)val->betScore / val->times;
	#ifdef ALG_CODE_FLOW
	printf("val->betScore:%ld, val->times:%d, totalBet:%f, line:%d in alg_kill_fish()\n", val->betScore, val->times, totalBet, __LINE__);
	#endif
	...
	while (loopTime){
		...
		if((gResult.prizeType == E_PRIZETYPE_FREEGAME) && (option > 0) && (option < OPTION_NUM))
		{
			...
			//printf("rotaryMulti:%d, line:%d in alg_kill_fish()\n", rotaryMulti, __LINE__);
			//#endif
			gResult.freegameMultis = rotaryMulti * val->betScore; 
			#ifdef ALG_CODE_FLOW
