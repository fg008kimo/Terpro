...
void dy_handle(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
...
#endif
	dyTotalRound++;
	dyTotalBet += (double)val->betScore / val->times;
	...
	if(ret->normalMulti > 0)
	{
		...
		dyNormalMinMulti = (dyNormalMinMulti > (double)ret->normalMulti / val->times) ? (double)ret->normalMulti / val->times: dyNormalMinMulti;
		dyNormalMaxMulti = ((double)ret->normalMulti / val->times> dyNormalMaxMulti) ? (double)ret->normalMulti / val->times : dyNormalMaxMulti;
		#ifdef NORMAL_WIN_NUM
	...
	if(ret->prizeType == E_PRIZETYPE_FREEGAME)
	{
		...
		dyFreeMinMulti = (dyFreeMinMulti > (double)freeMulti) ? (double)freeMulti / val->times : dyFreeMinMulti;
		dyFreeMaxMulti = ((double)freeMulti / val->times > dyFreeMaxMulti) ? (double)freeMulti / val->times : dyFreeMaxMulti;		
	}
	if(ret->prizeType == E_PRIZETYPE_RESPINGAME)
	{
		...
		dyRespinMinMulti = (dyRespinMinMulti > (double)respinMulti) ? (double)respinMulti / val->times : dyRespinMinMulti;
		dyRespinMaxMulti = ((double)respinMulti / val->times > dyRespinMaxMulti) ? (double)respinMulti / val->times : dyRespinMaxMulti;		
	}
...
STR_AlgKillFishResult* protocol_mammonKillFish_2(int wantTableId)//wantTableId = 1
{
	...
	val.betScore = 10000000000;//val.betScore / val.times = level bet score
	...
	gTotalBet[gMachineIndex] += (double)val.betScore / val.times;
