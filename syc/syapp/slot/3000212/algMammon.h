...
typedef struct{
	...
	s8 animeType;			   				  // 動畫类型：0-無動畫、1-有動畫
	u32 totalMulti;            				  // 总倍率（等于normalMulti+freegameMultis[]）

	// 基础牌面，所有中奖类型都有
	int *normalIcons;      	  				  // 图标列表（一维数组，长度15）
	u32 normalMulti;                     	  // 倍率
	...
	#ifdef CAL_PRIZE_NUM
	//int *przSymbolDist;	   	   //calculate_prize_symbol dist
	//int *przFgSymbolDist;	   //calculate_prize_symbol dist
	//int *przFgNumDist;	   	   //calculate_prize_num dist
	int (*prizeNumMulti)[PRIZE_SYMBOL_NUM];	   	   //calculate prize num multi
	#endif
...
typedef struct{
	...
	int freegameIcons[FREE_GAME_NUM][BOARD_ICON_NUM];
	int freegameMultis[FREE_GAME_NUM];
	int freegameLineInfos[FREE_GAME_NUM][BET_LINE_TOTAL][3];

	s16 normalLineNum;
	s16 freegameNum;
	s16 freegameLineNum[FREE_GAME_NUM];
	#ifdef CAL_PRIZE_NUM
	//int przSymbolDist[PRIZE_SYMBOL_NUM];
	//int przFgSymbolDist[FREE_GAME_NUM][PRIZE_SYMBOL_NUM];
	//int przFgNumDist[PRIZE_SYMBOL_NUM];
	int prizeNumMulti[FREE_GAME_NUM][PRIZE_SYMBOL_NUM];
	#endif
