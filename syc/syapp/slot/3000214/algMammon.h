...
typedef struct{
	int prizeType;			   				  // 中奖类型：0-普通游戏、1-輪盤游戏、2-重複游戏
	s8 set;								  		// 檔數
	s64 totalMulti;            			 	  // 总倍率

	// 基础牌面，所有中奖类型都有
	int *normalIcons;      	  				  // 图标列表（一维数组）
	int *normalpreIcons;      	  		  	  // 表演图标列表（一维数组）
	int *respinIcons;      	  				  // 图标列表（一维数组）
	s64 normalMulti;                       	  // 基礎游戏倍率
	s64 respinMulti;                       	  // 重轉游戏倍率 
	s64 freegameMultis;					  	  // 輪盤游戏倍率
...
typedef struct{
	...
	int normalpreIcons[BOARD_ICON_NUM];
	int respinIcons[BOARD_ICON_NUM];
	s64 normalMulti;
	s64 respinMulti;
	s64 freegameMultis;
