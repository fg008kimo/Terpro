...
#define RANDOM_ROUND 10000000000
...
{
	LOGI("odd/even=%f", (double)randomOddEven[1] / randomOddEven[0]);
}
...
void random_print_units()
{
	...
	for(units = 0; units < 10; units++)
		LOGI("[%d]=%f", units, (double)randomUnits[units] / RANDOM_ROUND);
}
...
{
	LOGI("bin_1/bin_0=%f", (double)randomBin[1] / randomBin[0]);
}
...
void test_random()
{
	...
	{
		random_handle(GetRand(RANDOM_MAX));
	}
	random_print();
}
