...
double get_totalWinMulti(s8 prizeType, double normalMulti, double freegameMultis, double respinMulti)
{
	double totalMulti = 0.0;

	#ifdef ALG_CODE_FLOW
	printf("normalMulti:%lf, freegameMultis:%lf, respinMulti:%lf, line:%d in alg_kill_fish()\n", normalMulti, freegameMultis, respinMulti, __LINE__);
	#endif
...
void alg_kill_fish(STR_AlgKillFish* val, STR_AlgKillFishResult* ret)
{
	...
	if(is_dajiang_mode()) //大奖模式
	{
		if(isBuyFreegame || GetRandom() >= 0.5)
			board_gen(gTableWeight, TABLE_NUM - 1, gBaseSymbolWeight, gBaseSymbolWeightNum, gMulWeight, gResult.normalIcons, 0, gResult.option);
		else
			board_gen(gTableWeight, TABLE_NUM - 1, gBaseSymbolWeight, gBaseSymbolWeightNum, gMulWeight, gResult.normalIcons, 0, gResult.option);
	}
	else
	...
	char *normalMultiBuf;
	normalMultiBuf = board_calMulti(gResult.normalIcons);
	gResult.normalMulti = atof(normalMultiBuf);
	double testNormal = gResult.normalMulti;
	#ifdef ALG_CODE_FLOW
	printf("normalMultiBuf:%s, gResult.normalMulti:%lf, line:%d in alg_kill_fish()\n", normalMultiBuf, gResult.normalMulti, __LINE__);
	#endif
	if(gResult.normalIcons[3] == E_ICON_SC1)
		gResult.prizeType = E_PRIZETYPE_FREEGAME;
	else if(gResult.normalIcons[3] == E_ICON_RE){
		...
		{
			if(isBuyFreegame || GetRandom() >= 0.5)
				board_gen(0, 0, gBaseSymbolWeight, gBaseSymbolWeightNum, 0, gResult.respinIcons, respin, gResult.option);
			else
				board_gen(0, 0, gBaseSymbolWeight, gBaseSymbolWeightNum, 0, gResult.respinIcons, respin, gResult.option);
		}
		else
		...
		char *respinMultiBuf;
		respinMultiBuf = board_calMulti(gResult.respinIcons);
		gResult.respinMulti = atof(respinMultiBuf);
		#ifdef ALG_CODE_FLOW
		printf("respinMultiBuf:%s, gResult.respinMulti:%d, line:%d in alg_kill_fish()\n", respinMultiBuf, gResult.respinMulti, __LINE__);
		#endif
		gResult.normalMulti = testNormal;
		#ifdef ALG_CODE_FLOW
		printf("gResult.normalMulti:%lf, line:%d in alg_kill_fish()\n", gResult.normalMulti, __LINE__);
		#endif
	}
	//if(isBuyFreegame && gResult.prizeType != E_PRIZETYPE_FREEGAME)
	//	LOGE("buyFreegame error: prizeType=%d", gResult.prizeType);
	if(gResult.prizeType == E_PRIZETYPE_FREEGAME)//betSize = 1 5 10 50 100
	{
		int i, rotaryMulti = 0;
		
		rotaryMulti = getRandomValue(gRotaryIcon, ROTARY_ICON_NUM, ROTARY_WEIGHT_SUM); //gen the number chosen from rotary board
		//#ifdef ALG_CODE_FLOW
		//printf("rotaryMulti:%d, line:%d in alg_kill_fish()\n", rotaryMulti, __LINE__);
		//#endif
		gResult.freegameMultis = rotaryMulti * val->betSize; 
		#ifdef ALG_CODE_FLOW
		printf("gResult.freegameMultis:%lf, line:%d in alg_kill_fish()\n", gResult.freegameMultis, __LINE__);
		printf("gResult.normalMulti:%lf, line:%d in alg_kill_fish()\n", gResult.normalMulti, __LINE__);
		#endif
	}
	...
	#ifdef ALG_CODE_FLOW
	printf("gResult.totalMulti:%lf, line:%d in alg_kill_fish()\n", gResult.totalMulti, __LINE__);
	#endif
